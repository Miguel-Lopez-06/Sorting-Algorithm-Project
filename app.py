...
if st.session_state.page == "Home Page":
    ...
    st.image("pics/lexical.png", caption="Programming Language Interpreter Flow", use_container_width=True)
...
class Lexer:
    ...
    st.subheader("Parser")
    st.write("The Parser consumes the token list generated by the lexer and checks that it matches the expected grammar and command structure. " \
    "If the token sequence is valid, the parser builds a data structure "
    "(like a dictionary or a custom Command object) capturing the user's intentions: which array to sort, which algorithm to use, and the desired ordering.")
    st.code("""
